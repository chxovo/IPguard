...
目前数据库已收集 29028 个恶意IP，上万级库持续更新中...
轻量使用推荐使用**blacklist.txt** 列表包含最新更新库
重度使用推荐使用**blacklist_all.txt** 列表包含网络收集20000+以及本地**blacklist.txt**合并在一起的恶意IP库。

 
项目说明
 
本仓库仅用于存储 IP 黑名单文本文件（blacklist.txt），所有文件均为纯文本格式，每行一个 IP 地址（可附带注释说明），方便直接导入防火墙或安全工具进行批量封禁。
